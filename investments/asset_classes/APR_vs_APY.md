...
The Annual Percentage Rate (APR) is an attempt at bundling fees and interest into an effective interest rate.  The interest rate part of the APR follows a very simple formula that mocks the annualized rate, without specifying the compounding schedule.  For a given rate over a period, the APR is simply rate per period times the number of periods in a year.  For example, if a loan compounds monthly at 1% interest, the APR for this loan is:  
...
$r_y = e^r - 1 \quad ,$  
which is strictly greater than $r$ (e.g., consider the Taylor expansion around $r=0$).
